...
def analyze(hist_data: pd.DataFrame) -> TechnicalAnalysis:
    ...
    sma50 = float(close.rolling(window=50).mean().iloc[-1]) if len(close) >= 50 else current_price
    sma200 = float(close.rolling(window=200).mean().iloc[-1]) if len(close) >= 200 else current_price
    ...
    # 4. Volume Trend (20%) - 20-day vs 50-day average
    vol20 = float(volume.rolling(window=20).mean().iloc[-1]) if len(volume) >= 20 else float(volume.iloc[-1])
    vol50 = float(volume.rolling(window=50).mean().iloc[-1]) if len(volume) >= 50 else float(volume.iloc[-1])
