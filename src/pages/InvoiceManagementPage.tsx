...
import { getSearchCount } from '../services/historyService'; // wait, we don't have getSearchCount in historyService, but we can query history list size.
      ...
      <main className="flex-1 max-w-7xl w-full mx-auto px-6 py-8 space-y-8 animate-fade-in">
        ...
        <div className="flex flex-col md:flex-row md:items-center md:justify-between border-b border-slate-800/60 pb-6 gap-4">
          <div>
            ...
            <p className="text-sm text-slate-400 mt-1">Review generated invoices and execute manual search billing runs.</p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-1 glass rounded-3xl p-6 space-y-5 flex flex-col justify-between">
            <div className="space-y-4">
              ...
              <div>
                <label className="block text-xs font-semibold text-slate-500 uppercase tracking-widest mb-2">User Database</label>
                <select
                  onChange={(e) => {
                  ...
                  className="w-full bg-slate-900 border border-slate-800 rounded-xl px-3 py-3 text-sm focus:outline-none text-slate-200 font-semibold"
                >
                  {users.map(u => (
            ...
            <div className="bg-slate-900/40 border border-slate-800/60 p-4 rounded-2xl flex items-start space-x-2.5">
          ...
          {/* Bill Preview Column */}
          <div className="lg:col-span-2 glass rounded-3xl p-6 flex flex-col justify-between relative overflow-hidden">
              ...
              {selectedUser ? (
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="space-y-1">
                    <span className="text-[10px] text-slate-500 uppercase tracking-wider block">Billed To</span>
                  ...
                  <div className="space-y-2 bg-slate-950/30 p-4 rounded-2xl border border-slate-800/40">
                    <div className="flex justify-between text-sm">
                      <span className="text-slate-400">Unbilled Searches:</span>
                      <span className="font-bold text-slate-200">{unbilledCount}</span>
                    </div>
                    <div className="flex justify-between text-sm">
                      <span className="text-slate-400">Billing Rate:</span>
                      <span className="font-bold text-slate-200">₹{billingRate.toFixed(2)}</span>
                    </div>
            ...
            <button
              ...
              className="mt-6 w-full py-3 rounded-xl bg-gradient-to-r from-blue-500 to-indigo-600 text-white font-bold text-sm tracking-wider hover:opacity-90 active:scale-[0.99] disabled:opacity-40 disabled:scale-100 transition-all cursor-pointer"
            >
              ...
              <div className="overflow-x-auto">
                <table className="w-full text-left border-collapse">
                  ...
                  <tbody className="divide-y divide-slate-800/60">
                    {invoices.map((inv) => (
                      <tr key={inv.id} className="hover:bg-slate-800/10 transition-colors duration-200">
                        <td className="py-4 px-6 font-bold text-blue-400 text-sm">{inv.invoice_number}</td>
                        <td className="py-4 px-6 font-semibold text-slate-200 text-sm">{getUserNameById(inv.user_id)}</td>
                        <td className="py-4 px-6 text-center text-slate-300 font-bold text-sm">{inv.total_searches}</td>
                        <td className="py-4 px-6 text-right font-black text-slate-200 text-sm">₹ {inv.amount.toLocaleString('en-IN', { minimumFractionDigits: 2 })}</td>
                        ...
                        <td className="py-4 px-6 text-right">
                          <button
                            ...
                            className="inline-flex items-center space-x-1.5 px-3 py-1.5 rounded bg-slate-800 hover:bg-slate-700 text-xs font-bold transition-all text-slate-300 cursor-pointer"
                            title="Download PDF"
                          >
                            <Download size={14} />
